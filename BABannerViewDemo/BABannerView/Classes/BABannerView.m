...
#define BABannerViewWidth self.bounds.size.width
#define BABannerViewHeight self.bounds.size.height

//固定创建3个imageView进行循环复用，性能体验最好
//每次给images赋值的时候进行判断当前是第几张图片，是第几页。ImageView在轮播的过程中要通过计算切换图片和页码
static int imagesCount = 3;

@interface BABannerView () <UIScrollViewDelegate>

/** 滚动视图 */
@property (nonatomic, weak) UIScrollView *scrollView;
/** 页控制 圆点 */
@property (nonatomic, weak) BAPageControl *pageCtrl;
/** 定时器,用weak */
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        BAPageControl *pageCtrl = [[BAPageControl alloc] init];
...
- (void)setup
{
    ...
    for (int i = 0; i < imagesCount; i++) {
        BAImageView *imageView = [[BAImageView alloc] initWithFrame:CGRectMake(i * BABannerViewWidth, 0, BABannerViewWidth, BABannerViewHeight)];
    ...
    self.scrollView.contentSize = CGSizeMake(imagesCount * BABannerViewWidth, 0);
    
    //pageCtrl的当前页背景颜色和默认颜色
//    self.pageCtrl.currentPageIndicatorTintColor = _indiatorColor;
    self.pageCtrl.pageIndicatorTintColor = [UIColor colorWithRed:207/255.0f green:207/255.0f blue:207/255.0f alpha:1];
...
{
    self.pageCtrl.currentPageIndicatorTintColor = color;
}
...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i < imagesCount; i++) {
        BAImageView *imageView = self.scrollView.subviews[i];
        imageView.frame = CGRectMake(i * BABannerViewWidth, 0, BABannerViewWidth, BABannerViewHeight);
    }
    //重写pageCtrl的布局，默认居中，可根据项目需求修改位置
    CGFloat pageCtrlW = 60;
    CGFloat pageCtrlH = 40;
    CGFloat pageCtrlX = (BABannerViewWidth - pageCtrlW) / 2;
    CGFloat pageCtrlY = BABannerViewHeight - pageCtrlH;
    self.pageCtrl.frame = CGRectMake(pageCtrlX, pageCtrlY, pageCtrlW, pageCtrlH);
    
    //重写titleLabel的布局
    CGFloat titleH = 40;
    self.titleLabel.frame = CGRectMake(0, BABannerViewHeight - titleH, BABannerViewWidth, titleH);
    //1. 修复bug,让其默认从第0页开始
    self.pageCtrl.currentPage = 0;
    BAImageView *imageView = self.scrollView.subviews[0];
    [imageView asynSetImage:self.slImages[0] placeholderImage:self.placeholderImg];
    //2. 修复bug, 让其加载完成，就展示第二个imageView
    self.scrollView.contentOffset = CGPointMake(BABannerViewWidth, 0);
...
{
    BAImageView *imageView = [[BAImageView alloc] initWithFrame:self.bounds];
...
- (void)setupContent
{
    ...
    for (int i = 0; i < self.scrollView.subviews.count; i++) {//3
        BAImageView *imageView = self.scrollView.subviews[i];//1. 0
...
- (void)nextPage
{
    ...
    [self.scrollView setContentOffset:CGPointMake(2 * BABannerViewWidth, 0) animated:YES];
    
}

#pragma mark - BABannerViewDelegate代理方法，监听点击的哪一个imageView
/**
 点击了哪一张图片
...
{
    BAImageView *imageView = (BAImageView *)tap.view;
